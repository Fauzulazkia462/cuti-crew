...
class Karyawan extends CI_Controller
{
    ...
    public function index()
    {
       ...
       $data['karyawan']=$this->db->query("select *, saldo_cuti.saldo_cuti as saldo, saldo_cuti.cuti_terpakai as cutter  from karyawan join saldo_cuti on karyawan.nik=saldo_cuti.nik")->result(); 
    ...
    {
		$data['report'] =$this->db->query("select a.nik AS nik, b.nama_karyawan AS nama_karyawan, a.tahun AS tahun, a.saldo_cuti AS saldo_cuti, a.cuti_terpakai AS cuti_terpakai, (a.saldo_cuti - a.cuti_terpakai) AS sisa_cuti from (cuticrew.saldo_cuti a join cuticrew.karyawan b) where (a.nik = b.nik) order by a.nik")->result();

        
			
							
		$this->load->view('ps/v_report', $data);					
	}

	public function report_resign()
    {
		$data['report']=$this->db->query("select * from v_cuti_resign")->result();
							
		$this->load->view('ps/v_report_resign', $data);					
	}	
    ...
    public function import_excel()
    {
        ...
        for ($i = 1; $i < count($schdeules); $i++) {
			...
			$cek_nik=$this->db->query("SELECT * FROM karyawan WHERE nik='$nik'");
    ...
    public function buat_user()
    {
        ...
        for ($i = 1; $i < count($schdeules); $i++) {
            $nik = $schdeules[$i]['0'];
            $tgl_masuk_kerja = $schdeules[$i]['11'];
            ...
            $cek_user=$this->db->query("SELECT * FROM users WHERE nik='$nik'");
    ...
    public function buat_saldocuti()
    {
        ...
        for ($i = 1; $i < count($schdeules); $i++) {
            ...
            $cek_saldo=$this->db->query("SELECT * FROM saldo_cuti WHERE nik='$nik'");
    ...
    public function do_insert_cuti_bersama()
    {
        ...
        if (!empty($tanggal)) {
            ...
            foreach ($join as $row){
				...
				$cek=$this->db->query("select count(*) as jumlah from cuti where nik='$db->nik' and tgl_mulai='$tanggal1' ")->result(); 
				foreach ($cek as $cek){
					$jml = $cek->jumlah;
					if ($jml == 0)
					{
	...
	public function history()
	{
			...
			$list_data = $this->db->query("select a.*, b.nama_karyawan from cuti as a, karyawan as b where a.nik=b.nik and a.nik='$nik'")->result();
			...
			$html .= '<tr>';
			$html .= '<th>No';
			$html .= ' &nbsp&nbsp&nbsp&nbsp&nbsp<th>Nama';
			$html .= '<th>Jenis';
			$html .= '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <th>Tanggal';
			$html .= '<th>Total';
			$html .= '';
			$totuang = 0;
			$i=1;
            $html .= '';
			foreach ($list_data as $employee) {
				
				$totuang = $totuang +$employee->total_hari;
				
				$html .= '<tr>';
				$html .= '<td>';
				$html .= $i;
				$html .= '';
				$html .= '<td>'.$employee->nama_karyawan.'';
				$html .= '&nbsp&nbsp&nbsp&nbsp&nbsp <td>'.$employee->jenis.'';
				$html .= '<td>'.$employee->tgl_mulai.' sd '.$employee->tgl_berakhir.'';
				$html .= '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <td>'.$employee->total_hari.'';
				$html .= '';
				$i++;
			}
			$html .= '<tr>';			
			$html .= '<td colspan=4 align=center><strong>Total Cuti';
			$html .= '<td><strong>'.$totuang.'';
			$html .= '';

			echo json_encode($html);

            // $this->load->view('ps/v_report', $list_data);
	}
	...
	public function pengajuan_cuti()
    {
		...
		$data['karyawan'] = $this->db->query("select nik, nama_karyawan from karyawan order by nama_karyawan ASC")->result();
        $this->load->view('ps/v_pengajuan_cuti_khusus', $data);
    }
    ...
    public function pengajuan_izin()
    {
        $data['atasan'] = $this->db->query("select nama_supervisor as atasan from supervisor order by nama_supervisor ASC")->result();
		$data['karyawan'] = $this->db->query("select nik, nama_karyawan from karyawan order by nama_karyawan ASC")->result();
        $this->load->view('ps/v_pengajuan_izin_khusus', $data);
    }
		
	public function pengajuan_cuti_khusus_do_insert()
    {
        //Input Form
        ...
        if (!empty($nik) && !empty($tgl_mulai) && !empty($tgl_berakhir) && !empty($keterangan)) {
            if ($row_n < 1) {
                ...
                redirect('ps/karyawan/pengajuan_cuti');
            } else {
                //$sendgrid->send($email);
                $this->m_Data->insert_data($data, 'cuti');
                $this->session->set_flashdata('msg',
    ...
    public function pengajuan_izin_khusus_do_insert()
    {
        //Input Form
        $atasan = $this->input->post('atasan');
        $nik = $this->input->post('nik');
        ...
        if (!empty($nik) && !empty($tgl_mulai) && !empty($tgl_berakhir) && !empty($keterangan)) {
            if ($row_n < 1) {
                ...
                redirect('ps/karyawan/pengajuan_izin');
            } else {
                //$sendgrid->send($email);
                $this->m_Data->insert_data($data, 'cuti');
                $this->session->set_flashdata('msg',
                    '<div class="alert alert-success">
                        <h4>Berhasil Submit Izin</h4>
                    </div>');
                redirect('ps/karyawan/pengajuan_izin');
            }
        } else {
...
//         }
    ...
    {
        $data['karyawan'] = $this->db->query("select a.* , b.nama_karyawan from users as a, karyawan as b where a.nik=b.nik and a.nik='$id_karyawan'")->result();
        $this->load->view('ps/v_reset_password', $data);
    }
...
<h4>Berhasil Update</h4>
	...
	function update_password(){
        ...
        $this->session->set_flashdata('msg',
        '<div class="alert alert-success">
		<h4>Berhasil Reset Password Baru</h4>
		</div>');
		redirect('ps/karyawan/');
