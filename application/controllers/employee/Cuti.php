...
class Cuti extends CI_Controller {
	...
	private function count_lama_bekerja(){
	...
	{	
		...
		$data['riwayat_cuti']=$this->db->query("select * from cuti where nik='$nik' and approval='ya' and jenis='cuti'")->result();
		$data['riwayat_cuti']=$this->db->query("select * from cuti where nik='$nik' and approval='ya' and jenis='izin'")->result();
	 ...
	 */
        ...
        // Email body content
        $mailContent = "Berdasarkan Data Dibawah Ini<br>NIK : ".$nik."<br>Nama : ".$nama."
						<br>Jenis Pengajuan : Cuti<br>Tanggal Mulai : ".$tgl_mulai."<br>Tanggal berahir : ".$tgl_berakhir."<br>
						Jumlah Hari : ".$jumlahhari."<br>Keterangan : ".$keterangan."<br>
						Silahkan Login <a href='https://cuticrew.niramasutama.com/'>Disini </a> Untuk Approval";
	 ...
	 foreach ($ceksaldo as $row){
		...
		if ($ceksaldo < 1){
			$this->session->set_flashdata('msg', 
			'<div class="alert alert-danger">
				<h4>Kuota Cuti Anda Sudah 0, Jika Urgent Silahkan Ajukan Izin !</h4>
			</div>');  
			redirect('employee/cuti');
