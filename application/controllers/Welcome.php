...
// $sendMessageRequest2 = new SendMessageRequest([
    ...
    public function coba1()
    {
        ...
        foreach ($cuti as $cuti) {
            ...
            if ($kerja == 0) {
                ...
                echo "berhasil update data " . $cuti->nik;
            }
        }
	...
	public function coba2()
    {
        $now = date('Y');
        $cuti_bersama = 0;
        $cuti = $this->db->query("SELECT * FROM cuti_bersama");
        foreach ($cuti->result() as $row) {
            if (strtotime(date('Y-m-d')) <= strtotime($row->tanggal)) {
                $cuti_bersama = $cuti_bersama + 1;
            }
        ...
        foreach ($cuti as $cuti) {
            ...
            $end_date = new DateTime(date("Y-m-d"));
            $interval = $start_date->diff($end_date);
            $tahun = (int) date('Y') - (int) substr($masukkerja, 0, 4);
            $kelipatan = $tahun / 4;
            $modulus = $tahun % 4;
            $kurang = substr($kelipatan, 0, 1);
				echo $kelipatan.'-'.$sesi.'-'.$modulus.'<br>';
    ...
    public function coba()
    {
        ...
        foreach ($cuti as $cuti) {
            ...
            $start_date = new DateTime($masukkerja);
            $end_date = new DateTime(date("Y-m-d"));
			//$end_date = new DateTime(date("2021-05-18"));
            $interval = $start_date->diff($end_date);
            $tahun = (int) date('Y') - (int) substr($masukkerja, 0, 4);
            $kelipatan = $tahun / 4;
            $modulus = $tahun % 4;
						
			$insert = round($interval->days / 365);		
            ...
            if ($modulus == 0) {
				...
				//echo $kelipatan.'-'.$modulus.'-'.$insert.'-'.$sesi.'-'.$kerja.'<br>';
            ...
            if ($insert > 0 && $kerja == 0) {
                ...
                echo "berhasil update data " . $cuti->nik.'<br>';
            }
        }
    }
